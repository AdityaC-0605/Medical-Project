...
def display_results(state):
    ...
    if state.structured_assessment:
        ...
        # Display sections in order - only show sections that have content
        sections = [
        ...
        if sections_generated == 0:
            st.info("Assessment generated but sections could not be parsed. Raw output may be available in logs.")
    else:
...
def main():
    ...
    with st.sidebar:
        st.markdown("### 📖 About")
        st.info("""
        This AI system analyzes medical images and provides:
